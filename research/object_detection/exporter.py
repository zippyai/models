...
def _add_output_tensor_nodes_ssd(output_tensors,
    ...
    Adds the following nodes for output tensors -
      ...
      * detection_classes: float32 tensor of shape [batch_size, num_boxes]
        containing class predictions for the detected boxes.
      * layer_19x19x512': float32 tensor of shape [batch, 19, 19, 512]
      * layer_10x10x1024': float32 tensor of shape [batch, 10, 10, 1024]
      * layer_5x5x512': float32 tensor of shape [batch, 5, 5, 512]
      * layer_3x3x256': float32 tensor of shape [batch, 3, 3, 256]
      * layer_2x2x256': float32 tensor of shape [batch, 2, 2, 256]
      * layer_1x1x128': float32 tensor of shape [batch, 1, 1, 128]
      * detection_masks: (Optional) float32 tensor of shape
        [batch_size, num_boxes, mask_height, mask_width] containing masks for each
        detection box.
    Args:
    ...
    masks = postprocessed_tensors.get('detection_masks')
    num_detections = postprocessed_tensors.get('num_detections')
    feature_maps = output_tensors.get('feature_maps')
    ...
    outputs['detection_classes'] = tf.identity(classes, name='detection_classes')
    outputs['num_detections'] = tf.identity(num_detections, name='num_detections')
    outputs['layer_19x19x512'] = tf.identity(feature_maps[0], name='layer_19x19x512')
    outputs['layer_10x10x1024'] = tf.identity(feature_maps[1], name='layer_10x10x1024')
    outputs['layer_5x5x512'] = tf.identity(feature_maps[2], name='layer_5x5x512')
    outputs['layer_3x3x256'] = tf.identity(feature_maps[3], name='layer_3x3x256')
    outputs['layer_2x2x256'] = tf.identity(feature_maps[4], name='layer_2x2x256')
    outputs['layer_1x1x128'] = tf.identity(feature_maps[5], name='layer_1x1x128')
...
def _export_inference_graph_ssd(input_type,
                                detection_model,
                                use_moving_averages,
                                trained_checkpoint_prefix,
                                output_directory,
    ...
    output_tensors = detection_model.predict(preprocessed_inputs)
    postprocessed_tensors = detection_model.postprocess(output_tensors)
    outputs = _add_output_tensor_nodes_ssd(output_tensors,
                                           postprocessed_tensors,
                                           output_collection_name)
...
def export_inference_graph_ssd(input_type,
                               pipeline_config,
                               trained_checkpoint_prefix,
                               output_directory,
                               optimize_graph=False,
    ...
    detection_model = model_builder.build(pipeline_config.model,
                                          is_training=False)
    _export_inference_graph_ssd(input_type, detection_model,
                                pipeline_config.eval_config.use_moving_averages,
